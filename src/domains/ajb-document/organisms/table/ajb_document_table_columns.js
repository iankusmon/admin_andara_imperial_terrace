 ...
 */
const ajbdocumentTableColumns = ({
  buttonText,
  buttonColour,
  onButtonClick
}) => {

  const AjbDocumentStatusFilter = ({ column }) => {
  ...
  const AjbDocumentStatusCell = (row) => <StatusBadge status={ row.cell.value } />
  ...
  return (
    [
      ...
      },
      {
        Header   : 'Status',
        accessor : 'status',
        Filter   : AjbDocumentStatusFilter,
        Cell     : AjbDocumentStatusCell
      },
      {
        Header : 'Action',
        id     : 'action',
...
ajbdocumentTableColumns.propTypes = propTypes

export default ajbdocumentTableColumns
